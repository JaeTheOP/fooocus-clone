# KLEVR 2.1.1 Central Pricing Manager
...
## 2.1.1 packaging correction

- Payload checksum keys are relative to the payload root and omit the `payload/` prefix.
- Each migration declares the required `id` and `up` fields.
- Migration and health-check checksums remain package-root relative.

## Installation

1. Discard the rejected 2.1.0 staged package.
2. Open **Admin > Software Updates**.
3. Upload `klevr-software-update-2.1.1-pricing-manager.zip`.
4. Confirm the current version is at least 1.9.9 and the target version is 2.1.1.
5. Choose **Back Up & Apply**.
6. Confirm all health checks pass.
7. Open **Admin > Pricing Manager**.
8. Change one package or equipment price and verify the same price on Packages, System Builder, cart, and checkout.
